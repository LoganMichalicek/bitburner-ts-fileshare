...
export const colorText: { [K in ColorsType]: (text: string) => string } =
  Object.keys(colors).reduce((acc, key) => {
    ...
    return acc;
  }, {} as { [index: string]: (text: string) => string }) as {
    [K in ColorsType]: (text: string) => string;
  };

export const colorBackground: {
  [K in BackgroundColorsType]: (text: string) => string;
